...
bool Model3DS::_Load()
{
  ...
  mnDisplayList[0] = _GenerateDisplayList( pModel, 0 ); // polygon
  mnDisplayList[1] = _GenerateDisplayList( pModel, 1 );  // xray
 ...
 */
GLuint Model3DS::_GenerateDisplayList( Lib3dsFile * pModel, int type )
{
  assert( pModel );

  //some defaults
  static GLfloat ambient[4] = {0.2f, 0.2f, 0.2f, 0.7f};
  static GLfloat diffuse[4] = {0.8f, 0.8f, 0.8f, 0.7f};
  static GLfloat spectal[4] = {0.2f, 0.2f, 0.2f, 0.7f};
  ...
  if ( list !=0 )
  {
    ...
    for(int mm = 0; mm < pModel->nmeshes; mm++ )
    {
      ...
      for( int ff = 0; ff < mesh->nfaces; ff++ )
      {
        ...
        if( type == 0 && face.material >= 0)
        {
            
          Lib3dsMaterial * material = pModel->materials[ face.material ];
            
          // keep track of textures
            if (texVals[face.material] != -1) {
                glEnable(GL_TEXTURE_2D);
                glBindTexture (GL_TEXTURE_2D, texVals[face.material] );
            }
        ...
        for( int vv = 0; vv < 3; vv++ )
        {
          ...
          // Give OpenGL the texture coordinate for this vertex
          glTexCoord2f (mesh->texcos[ index ] [0], 
                         1 - mesh->texcos[ index ] [1]);  // me thinks...
            
          // Draw in the current vertex of the object (Corner of current face)
          glVertex3fv( mesh->vertices[ index ] );
          
            if (type == 0) {
                // Store the vertices for easy access
                Vector<3> b = makeVector(  mesh->vertices[ index ] [0], 
...
{
  double inc = GV3::get<double>("ModelsGame.MoveIncrement", "0.03", SILENT| HIDDEN);
...
void Model3DS::_LoadTextures( Lib3dsFile * pModel )
{
  assert( pModel );
    ...
    GLuint* texValsFile = new GLuint[pModel->nmaterials];
    texVals = vector<int>(pModel->nmaterials);
                                   
  for( int ii = 0; ii < pModel->nmaterials; ++ii) // go through all the materials in the model
  {
    ...
    if( !sTexFile.empty() ) {
        ...
        ss << "ARData/Textures/" << sTexFile;
        
        cout << "Opening texture: " << sTexFile << endl;
        Image<Rgba<byte> > in;
        in = img_load(ss.str());
        
        // get texture number from opengl
        glGenTextures(pModel->nmaterials, texValsFile);
        
        texVals[ii] = (int) texValsFile[ii];
        cout << "texVal" << texValsFile[ii] << endl;
        
        glBindTexture(GL_TEXTURE_2D, texValsFile[ii]); 
        // generic alignment, not sure if this is necessary
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1); 
        
        // values to modify the extent of the texture
