...
LINKFLAGS = -framework OpenGL -framework VecLib -L $(HOME)/local/lib -lGVars3 -lcvd $(3DSLIB) -m32 

# Edit this line to change video source
VIDEOSOURCE = VideoSource_OSX.o
...
PTAMM: $(OBJECTS)
	...
	/usr/bin/Rez OSX_resource_file.r -o PTAMM

CameraCalibrator:$(CALIB_OBJECTS)
	$(CC) -o CameraCalibrator $(CALIB_OBJECTS) $(LINKFLAGS)
	/usr/bin/Rez OSX_resource_file.r -o CameraCalibrator
...
depend:
	...
	makedepend -fdependencies $(INCLUDEFLAGS) $(MOREINCS) *.cpp *.cc *.h
